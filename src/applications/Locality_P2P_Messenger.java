...
public class Locality_P2P_Messenger extends Application{
    ...
    /** Application ID */
    public static final String APP_ID = "cs.standrews.ac.uk.Locality_P2P_Messenger";
    ...
    public Locality_P2P_Messenger(Settings s) {
    ...
    public Locality_P2P_Messenger(Locality_P2P_Messenger em) {
        super(em);
    ...
    public Message handle(Message msg, DTNHost host) {
        ...
        if (msg.getTo() == host && type.equalsIgnoreCase("encryptedMsg")) {
            ...
            if (unencryptedMessage.equalsIgnoreCase(unencryptedMsgOriginal)) {
                super.sendEventToListeners("RH:" + original_from.getName(), null, host);

                host.getMyMessages().add(msg.getId());
                String id = "encryptedResponse" + SimClock.getIntTime() + "-" + host.getAddress();
                Message m = new Message(host, original_from, id, getResponseSize());
                byte[] encryptedAES = rsa_aes_encryption.encryptAESwithRSA(
                        host.getSecretKeys().get(original_from),
                ...
                m.addProperty(
                        "contents",
                        rsa_aes_encryption.encryptMessageWithAES("success" + msg.getId(), host.getSecretKeys().get(original_from))
                );
                m.setSize(1000000);
                m.addProperty("time", rsa_aes_encryption.encryptMessageWithAES("" + m.getCreationTime(), host.getSecretKeys().get(original_from)));
                m.setTimeCreated(0);
    ...
    public Application replicate() {
        return new Locality_P2P_Messenger(this);
    }
